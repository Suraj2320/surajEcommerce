...
import { categories, getFeaturedProducts, getProductsByCategory } from "../data/products.js";
    ...
    return (<div className="min-h-screen">
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-6">
                <div className="grid md:grid-cols-2 gap-8 items-center">
                    ...
                    <div className="hidden md:grid grid-cols-2 gap-4">
                        {categories.slice(0, 4).map((cat) => (<Link key={cat.id} href={`/category/${cat.slug}`}>
        ...
        <section className="py-8 border-b">
            <div className="max-w-7xl mx-auto px-4 md:px-6">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    {features.map((feature, i) => (<div key={i} className="flex items-center gap-3" data-testid={`feature-${i}`}>
        ...
        <section className="py-12 bg-card">
            <div className="max-w-7xl mx-auto px-4 md:px-6">
                ...
                <ProductGrid products={featuredProducts.slice(0, 8)} columns={4} isLoading={false} />
            </div>
        </section>

        {categories.slice(0, 4).map((category) => {
            const categoryProducts = getProductsByCategory(category.slug).slice(0, 4);
            return (<section key={category.id} className="py-12">
                <div className="max-w-7xl mx-auto px-4 md:px-6">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                    ...
                    <ProductGrid products={categoryProducts} columns={4} isLoading={false} />
                </div>
            </section>);
        })}
